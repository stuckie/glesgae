...
static ALfloat calculate_distance_attenuation(const ALCcontext *ctx, const ALsource *src, ALfloat distance)
{
    ...
    switch (ctx->distance_model) {
        ...
        case AL_EXPONENT_DISTANCE:
            ...
            return (float)SDL_pow(distance / src->reference_distance, -src->rolloff_factor);
...
static void calculate_channel_gains(const ALCcontext *ctx, const ALsource *src, float *gains)
{
    ...
    */
    ...
    if (has_sse) {
        ...
        radians = (mags == 0.0f) ? 0.0f : (float)SDL_acos(dotproduct_sse(at_sse, rotated_sse) / mags);
    ...
    if (has_neon) {
        ...
        radians = (mags == 0.0f) ? 0.0f : (float)SDL_acos(dotproduct_neon(at_neon, rotated_neon) / mags);
...

    {
        ...
        radians = (mags == 0.0f) ? 0.0f : (float)SDL_acos(dotproduct(at, rotated) / mags);
